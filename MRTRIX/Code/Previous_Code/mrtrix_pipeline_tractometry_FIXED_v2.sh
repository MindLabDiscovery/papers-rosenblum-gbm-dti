...
while IFS= read -r TRACT_NAME; do
    ...
    if ! tcksample "$TRACT_RESAMPLED" "$FA" "$FA_TXT" -stat_tck mean -force -quiet < /dev/null 2>"$STATS_DIR/${TRACT_NAME}_error.log"; then
        echo " WARNING: tcksample FA failed for $TRACT_NAME, see ${TRACT_NAME}_error.log — skipping"
        continue
    fi
    tcksample "$TRACT_RESAMPLED" "$MD" "$MD_TXT" -stat_tck mean -force -quiet < /dev/null || { echo " WARNING: MD failed for $TRACT_NAME"; continue; }
    tcksample "$TRACT_RESAMPLED" "$AD" "$AD_TXT" -stat_tck mean -force -quiet < /dev/null || { echo " WARNING: AD failed for $TRACT_NAME"; continue; }
    tcksample "$TRACT_RESAMPLED" "$RD" "$RD_TXT" -stat_tck mean -force -quiet < /dev/null || { echo " WARNING: RD failed for $TRACT_NAME"; continue; }
