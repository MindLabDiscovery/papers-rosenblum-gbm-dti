% Paired t-test AND Wilcoxon with FDR correction

masterFile = '/Users/rosenble/Desktop/GBM/Glioblastoma/Data/Tract_Overall_Stats_V2/GBM_DTI_Master_Compiled.xlsx';
outPath    = '/Users/rosenble/Desktop/GBM/Glioblastoma/Data/Tract_Overall_Stats_V2/T-Test_Wilcoxon_Results_FDR.xlsx';
...
% Store raw p-values for all metrics before FDR
...
for m = 1:length(metrics)
    ...
    for t = 1:length(tracts)
        ...
        [~, p_t] = ttest(ipsi_vals, contra_vals);
        p_w      = signrank(ipsi_vals, contra_vals);
...
% Apply FDR per metric across all tracts
...
for t = 1:length(tracts)
    ...
    T_ttest.p_FA(t)     = all_p_ttest(t, 1);
    T_ttest.p_FA_FDR(t) = fdr_ttest(t, 1);
    T_ttest.p_MD(t)     = all_p_ttest(t, 2);
    T_ttest.p_MD_FDR(t) = fdr_ttest(t, 2);
    T_ttest.p_AD(t)     = all_p_ttest(t, 3);
    T_ttest.p_AD_FDR(t) = fdr_ttest(t, 3);
    T_ttest.p_RD(t)     = all_p_ttest(t, 4);
    T_ttest.p_RD_FDR(t) = fdr_ttest(t, 4);

    T_wilcoxon.p_FA(t)     = all_p_wilcoxon(t, 1);
    T_wilcoxon.p_FA_FDR(t) = fdr_wilcoxon(t, 1);
    T_wilcoxon.p_MD(t)     = all_p_wilcoxon(t, 2);
    T_wilcoxon.p_MD_FDR(t) = fdr_wilcoxon(t, 2);
    T_wilcoxon.p_AD(t)     = all_p_wilcoxon(t, 3);
    T_wilcoxon.p_AD_FDR(t) = fdr_wilcoxon(t, 3);
    T_wilcoxon.p_RD(t)     = all_p_wilcoxon(t, 4);
    T_wilcoxon.p_RD_FDR(t) = fdr_wilcoxon(t, 4);
end

writetable(T_ttest,    outPath, 'Sheet', 'Paired_tTest');
writetable(T_wilcoxon, outPath, 'Sheet', 'Wilcoxon');
